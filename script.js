...
document.getElementById('playlist').innerHTML += PLAYLIST.map((el, i) => {
  return `<article class="pl" onclick="play(${i})"><p><span class="pl-title">${
    el.track
  }. ${el.title}</span><br/><span class="pl-artist">${
    el.artist || 'ㅇㅇ'
  }</span></p></article>`;
}).join('');
...
function play(num) {
  ...
  document
    .querySelectorAll(
      `article.pl[onclick="play(${PLAYLIST.current})"]>p>span`
    )[0]
    .setAttribute('class', 'pl-title');
  document
    .querySelectorAll(
      `article.pl[onclick="play(${PLAYLIST.current})"]>p>span`
    )[1]
    .setAttribute('class', 'pl-artist');
  ...
  document.getElementById(
    'src-url'
  ).innerText = `https://www.youtube.com/watch?v=${PLAYLIST[num].id}`;
  document
    .getElementById('src-url')
    .setAttribute(
      'href',
      `https://www.youtube.com/watch?v=${PLAYLIST[num].id}`
    );
  ...
  player.__interval = setInterval(function () {
    ...
    document.getElementById('progress').innerText = minsec(
      player.playerInfo.currentTime - PLAYLIST[num].start
    );
  }, 100);
}
